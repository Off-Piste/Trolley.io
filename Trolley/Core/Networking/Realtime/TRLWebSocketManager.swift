...
class TRLWebSocketManager {
    ...
    func queueManager() {
        NotificationCenter.default.addObserver(.websocketCreated, object: nil) {
            TRLCoreNetworkingLogger.debug("\($0.name.rawValue) Observed")
            self.sendQueue()
        }
    }
    ...
    func sendQueue() {
        ...
        TRLCoreNetworkingLogger.debug("Attempting to send queue")
        ...
        do {
            ...
            if self.connected {
                ...
                TRLCoreNetworkingLogger.debug(json.rawString() ?? "")
                self.connection.send(json.rawString() ?? "")
                dm.clear()
            }
        } catch {
            TRLCoreNetworkingLogger.debug("Queue Is Empty")
        }
    }
    
    func addToQueue(_ message: String) {
        let dm = DefaultsManager(withKey: "WebSocketQueue")
        TRLCoreNetworkingLogger.debug("Adding Message [\(message)] to queue")
...
extension TRLWebSocketManager: TRLWebSocketDelegate {
    ...
    func webSocket(_ connection: TRLWebSocketConnection, onTextMessage message: String) {
        TRLCoreNetworkingLogger.debug("onMessage: \(message)")
    }
    
    func webSocket(_ connection: TRLWebSocketConnection, onMessage message: JSON) {
        TRLCoreNetworkingLogger.debug("onMessage: \(message.rawString() ?? "")")
    ...
    {
        TRLCoreNetworkingLogger.debug("onDisconnect [wasEverConnected: \(connected)]")
    ...
    func webSocketOnConnection(_ connection: TRLWebSocketConnection) {
        ...
        TRLCoreNetworkingLogger.debug("onConnection \(connection)")
    ...
    func webSocketOnLostConnection(_ connection: TRLWebSocketConnection) {
        TRLCoreNetworkingLogger.debug("onLostConnection \(connection)")
        self.connected = false
    }
    ...
    func attemptToRestablishConnection() {
        ...
        TRLCoreNetworkingLogger.debug("Attempting to reconnect to server")
